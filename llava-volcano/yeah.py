...
def start_predicting(annotations):
    ...
    for index_annotation, annotation in enumerate(annotations):
        ...
        for index_attribute, attribute in enumerate(annotation['attributes']):
            ...
            result = predict(annotation, attribute, prompt)
...
def predict(annotation, attribute, prompt: str) -> bool:
    args = type('Args', (), {
        "model_path": model_path,
        "model_base": None,
        "model_name": get_model_name_from_path(model_path),
        "query": prompt,
        "conv_mode": None,
        "image_file": annotation["filepath"],
        "sep": ",",
        "temperature": 0,
        "top_p": None,
        "num_beams": 1,
    ...
    return evaluate_prediction(prediction, attribute['answer'])
